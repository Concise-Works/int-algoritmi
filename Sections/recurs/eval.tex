...
\begin{Proof}[Iterative Substitution Method (plug-and-chug) - MergeSort]
    ...
    \begin{align*}
        ...
        T(n) &= 2\left[\hspace{3em} \right] + \Theta(n)\text{; Prepare to substitute the recurrence $T(n/2)$}\\
         &= 2\left[2T(n/2^2) + \Theta(n/2)\right] + \Theta(n);\text{ we evaluate $\Theta\left(2n/2\right)$, hence $\Theta(n)$}\\
...
\begin{align*}
    T(n) &= 2^kT(n/2^k) + k(\Theta(n))\text{; General form}\\
\end{align*}
...
\begin{align*}
    ...
    &= n\Theta(1) + \log_2 n\cdot\Theta(n);\\
