...
\begin{Answer}
    ...
    \begin{enumerate}
        ...
        we find that the expression becomes $n^{\log_3 2} + n \log_3n$. Since $n^{\log_3n} = \Theta(n)$, the result simplifies further to $n^{\log_3 2} + O(n)$. Noting that $n^{\log_3 2} < O(n)$ (because $\log_3 2$ is a fraction), the final result is $O(n)$.
